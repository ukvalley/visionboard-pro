...
const UserManagementPage = () => {
  ...
  const handleLoginAs = async (userId, userName) => {
    if (!window.confirm(`Are you sure you want to log in as ${userName}? You will be able to see their data and make changes on their behalf.`)) {
      return;
    }
                    ...
                    <tr key={user._id} className="border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                      ...
                      <td className="py-4 px-6 text-right">
                        <div className="flex items-center justify-end gap-1">
                          {/* Login As Button - Only for regular users (not admins) and not current user */}
                          {user._id !== currentUser?._id && user.role !== 'admin' && (
                            <button
                              onClick={() => handleLoginAs(user._id, user.name)}
                              disabled={loginAsLoading === user._id}
